...
# tpPrf <- profile_heatmap(
#   profileMat = matList[[TF_sample]][peak_genes$gene, ],
#   signalName = TF_sample,
#   geneGroups = peak_genes,
#   profileColor = profileColors[[TF_sample]],
#   ylimFraction = profileYlims[[TF_sample]]
# )
# 
# tpHt <- signal_heatmap(
#   log2_matrix = expressionData$log2_mat[peak_genes$gene, 1, drop = FALSE],
#   col_title = polII_sample,
#   column_title_gp = gpar(fontsize = 12),
...
# )
# 
# peaks_gl <- gene_length_heatmap_annotation(bedFile = file_genes,
#                                            genes = peak_genes$gene)
# 
# peaks_htlist <- peaks_gl$an + tpPrf$rowAnno + tpPrf$heatmap + tpHt
...
## decorate the annotations
add_annotation_titles(annotations = c("is_SM_gene", "is_TF", isExpCol, tssPeakTypeCol, tesPeakTypeCol),
                      anTitle = anLables)
